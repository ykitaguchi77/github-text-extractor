...
def paths_to_xml(paths, exclude_dirs=None):
   if exclude_dirs is None:
       exclude_dirs = []
   root_element = ET.Element("files")
   for path in paths:
       if os.path.isfile(path):
           file_element = ET.SubElement(root_element, "file", name=os.path.basename(path))
...
def main():
   ...
   parser.add_argument('--readme', action='store_true', help='Extract the README file')
   parser.add_argument('--code', action='store_true', help='Extract code files')
   ...
   try:
       ...
       if args.readme and args.code:
           readme_and_code_files = []
           readme_path = os.path.join(clone_dir, 'README.md')
           if os.path.exists(readme_path):
               readme_and_code_files.append(readme_path)
           for root, dirs, files in os.walk(clone_dir):
               for file in files:
                   if file.endswith(('.py', '.java', '.c', '.cpp', '.h', '.hpp', '.js', '.css', '.html')):
                       readme_and_code_files.append(os.path.join(root, file))
           root_element = paths_to_xml(readme_and_code_files, exclude_dirs)
       elif args.readme:
           readme_path = os.path.join(clone_dir, 'README.md')
           if os.path.exists(readme_path):
               root_element = paths_to_xml([readme_path], exclude_dirs)
           else:
           ...
           root_element = paths_to_xml(code_files, exclude_dirs)
       else:
